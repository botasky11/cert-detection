...
\begin{longtable}[]{@{}L{0.16}R{0.11}L{0.24}L{0.41}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.24}R{0.12}L{0.56}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.24}L{0.26}L{0.42}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.25}R{0.10}R{0.13}L{0.44}@{}}
\toprule\noalign{}
...
\begin{equation}
\begin{aligned}
\operatorname{score}(\mathrm{pattern})
&= \log(1 + \mathrm{hits})
   \cdot \operatorname{len}(\mathrm{pattern})^{1.5} \\
&\quad \cdot \left(
1 + \sum \operatorname{threat\_weight}(\mathrm{token})
\right)
\end{aligned}
\end{equation}
...
\begin{aligned}
\operatorname{score} =
&0.4 \cdot \operatorname{LCS\_ratio} \\
&+ 0.6 \cdot \operatorname{weighted\_high\_sensitive\_hit}
\end{aligned}
\end{equation}
...
$\ge 1.5$, computing IDF-weighted deviation scores:

\begin{equation}
\begin{aligned}
\operatorname{score} =
&\sum \log\left(1 + \operatorname{count}_u(\mathrm{token})\right) \\
&\cdot \operatorname{IDF}(\mathrm{token})
 \cdot \operatorname{threat\_weight}(\mathrm{token})
\end{aligned}
\end{equation}
...
\begin{aligned}
R(u) =
&0.40 \cdot z(\mathrm{rule})
+ 0.20 \cdot z(\mathrm{perplexity}) \\
&+ 0.25 \cdot z(\mathrm{similarity})
+ 0.15 \cdot z(\mathrm{peer})
\end{aligned}
\end{equation}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.18}L{0.18}R{0.10}L{0.46}@{}}
\toprule\noalign{}
...
h_v^{(l+1)} =
&\sigma\left(
W_{\mathrm{self}} h_v^{(l)}
+ \sum_r \sum_{u \in \mathcal{N}_r(v)}
\frac{w_{u,v,r}}{c_{v,r}} W_r h_u^{(l)}
\right)
\end{aligned}
\end{equation}

where $W_r$ employs basis decomposition:
\begin{equation}
W_r = \sum_{b=1}^{B} a_{rb} V_b
\end{equation}
sharing $B=4$ basis matrices across 12 relations to effectively reduce
parameter count.
...
\begin{tikzpicture}[
  ...
  input/.style={box, fill=blue!6, text width=32mm},
  layer/.style={box, fill=green!8, text width=38mm},
  head/.style={box, fill=orange!10, text width=34mm},
  arrow/.style={-{Latex[length=2.2mm]}, thick}
]
\node[input] (user) {user\\Linear$(6,64)$};
\node[input, below=of user] (pc) {pc\\Embedding$(64)$};
\node[input, below=of pc] (file) {file\\Embedding$(64)$};
\node[input, below=of file] (url) {url\\Embedding$(64)$};

\node[layer, right=of pc] (rgcn1) {R-GCN Layer 1\\$64 \rightarrow 64$, ReLU};
\node[layer, right=of rgcn1] (rgcn2) {R-GCN Layer 2\\$64 \rightarrow 64$};
\node[head, right=of rgcn2] (linear1) {User nodes only\\Linear$(64,32)$};
\node[head, right=of linear1] (linear2) {ReLU\\Linear$(32,1)$};
\node[head, right=of linear2] (out) {Sigmoid\\$P(\mathrm{malicious})$};

\draw[arrow] (user.east) -- (rgcn1.west);
\draw[arrow] (pc.east) -- (rgcn1.west);
\draw[arrow] (file.east) -- (rgcn1.west);
\draw[arrow] (url.east) -- (rgcn1.west);
\draw[arrow] (rgcn1) -- (rgcn2);
\draw[arrow] (rgcn2) -- (linear1);
\draw[arrow] (linear1) -- (linear2);
\draw[arrow] (linear2) -- (out);
\end{tikzpicture}%
}
\caption{R-GCN model structure.}
\label{fig:rgcn-model-structure}
\end{figure}
...
\item
  ...
  $\mathrm{pos\_weight} = \#\mathrm{neg} / \#\mathrm{pos} \approx 13.3$;
\item
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.28}R{0.18}R{0.18}R{0.20}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}R{0.10}R{0.13}R{0.13}R{0.13}R{0.13}R{0.11}R{0.11}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.09}L{0.20}R{0.09}R{0.11}R{0.11}R{0.12}R{0.12}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}L{0.38}R{0.22}R{0.22}@{}}
\toprule\noalign{}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}R{0.08}L{0.22}L{0.24}R{0.16}R{0.12}@{}}
\toprule\noalign{}
